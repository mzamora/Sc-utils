...
doplots=0;
% soundings at Antofagasta
%%
for yy=2016:2020
    mms=1:12;
    if yy==2020; mms=1:4; end
    for mm=mms
       for dd=1:eomday(yy,mm)
            for hh=[0 12]
                pathName=['SCFA/csv/85442_',num2str(yy),'_',sprintf('%.2d',mm),'_0100_',num2str(eomday(yy,mm)),'12.csv'];
                ...
                print(['SCFA/out/',datestr(date_i(ind),'yyyymmdd_hh')],'-dpng')
                end
                ind=ind+1;
...
ALL_SOUNDINGS.Properties.Description='Sounding parameters for SCFA, Jan 2016-Apr 2021';
...
save('SCFA_soundings_table','ALL_SOUNDINGS')
writetable(ALL_SOUNDINGS,'SCFA_soundings_table.csv')