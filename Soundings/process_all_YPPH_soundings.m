...
for yy=2014:2017
    for mm=[1,2,3,11,12];
        ...
        for dd=1:dmax
            for hh=[0 12]
                pathName=[stName,'/csv/',stNum,'_',num2str(yy),'_',sprintf('%.2d',mm),'_0100_',sprintf('%.2d',dmax),'12.csv'];
...
%                 text(60,1900,strcat('Δθ=',num2str(dtheta_decoupling(ind))))
                if ~isnan(zb_index)
                    print([stName,'/imgs/',datestr(date_i(ind),'yyyymmdd-hh')],'-dpng')
                end
                ind=ind+1;
